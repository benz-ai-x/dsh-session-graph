---
description: "可安装的 dsh Web 插件，用自由画布浏览、排列、分支并总结相关工作区会话。"
kind: "package-bundle"
---
...
本包为 DeepSeek Harness 对话视图添加 **Graph** 标签。它把通过 Branch（分支）连接的 Canvas Session（画布会话）组织成可移动、可折叠的 Session Cluster（会话簇），以有向边绘制 Branch 关系，并把 Subagent Session（子代理会话）折叠为 Subagent Summary（子代理摘要）。浏览器按图谱范围保存每份 Session Arrangement（会话排列）。图谱投影不会改变会话日志；“开新分支”委托 Harness 创建会话，“生成摘要”则发起一次独立、显式、只读的模型请求，结果绝不写回会话。
...
`pnpm run check` 会检查独立包的类型、构建 Host 与浏览器入口，并运行 94 项包内测试。若要对已准备好的 DeepSeek Harness checkout 运行 83 项 Host 与完整交互集成测试：
...
| 导出 | 用途 |
|---|---|
| `.` | 用于显式生成 Session Digest 的 Cordis Host service |
| `./invariant` | 运行时注册不变量 |
...
`GraphView` 读取 Viewed Session、Workspace 成员关系、会话摘要与待处理交互映射。纯 helper 推导 Session Cluster、Branch 边、Subagent Summary、布局、吸附、Title Filter 匹配与视口状态，再由 `GraphCanvas` 渲染结果。Host service 会检查显式指定的 Session，准备有上限的安全输入，调用选定 LLM 路由，验证结构化输出，再通过包自有 Remote 暴露结果，全程不修改持久化数据。
...
| [`src/index.ts`](src/index.ts) | Session Digest Host service、配置、Harness LLM 请求与 Remote 错误 |
