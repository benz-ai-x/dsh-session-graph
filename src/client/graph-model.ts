/**
 * Pure derivation of the workspace session graph from the sessions-list
 * snapshot: workspace-scope resolution, session-cluster partitioning, fork
 * edge rules, and the subagent badge aggregation. Subagent-origin sessions
...
/**
 * The workspace scope the graph renders. Membership follows the host's own
 * rule — `sessionIds` accounting is authoritative — with two display-only
...
export interface WorkspaceScope {
  /** Workspace title, or undefined for a cwd-only bucket. */
...
export interface GraphNode {
  ...
  /** The branch parent session id for attached fork children. */
  readonly branchFrom: SessionId | undefined
}

/** One fork edge between two attached canvas rows. */
...
/**
 * One session cluster: the derivation tree rooted at an ordinary session —
 * the root itself, its attached fork descendants, and every folded subagent
 * chain hanging off them. Isolated sessions form single-member clusters.
 */
...
export interface SessionGraph {
  ...
  /** Attached fork children in display order (updatedAt descending, id tiebreak). */
  readonly children: ReadonlyMap<string, readonly string[]>
  readonly edges: readonly GraphEdge[]
  readonly workspaceLabel: string | undefined
  /** Count of canvas nodes (the workspace's visible sessions). */
  readonly sessionCount: number
}

/**
 * Resolve the workspace scope the graph of one viewed session renders.
...
export function resolveWorkspaceScope(
  ...
  return { label: workspace?.title, path, members }
}
 ...
 * it reaches through an uninterrupted subagent-origin chain, and an ordinary
 * fork terminates propagation (a fork child's subagents belong to the fork's
 * own subtree). Cycles fail soft via the seen set.
 ...
 *
 * Rules: blank rows are excluded except the current one; subagent-origin
 * rows never render as canvas nodes (they fold into ancestor badges); a
 * fork edge requires the parent to be a visible non-subagent row (a fork
 * taken inside a subagent renders as a new root in its own cluster); every
 * canvas node carries the badge totals of its own uninterrupted subagent
 * chain. Clusters partition the canvas rows by derivation-tree root.
 * @param list - the sessions-list snapshot.
 * @param scope - the resolved workspace scope, or undefined for the empty state.
 * @param currentId - the viewed session (kept when blank, highlighted).
...
export function deriveSessionGraph(
  ...
  if (scope !== undefined) {
    for (const row of Object.values(list.byId)) {
      ...
      if (row.blank && row.id !== currentId) continue
  ...
  const forkChildrenOf = (row: SessionSummary): SessionSummary[] =>
  ...
  const placeMember = (row: SessionSummary, clusterRootId: SessionId, members: SessionId[]): void => {
    ...
    nodes.set(row.id, {
      ...
      blank: row.blank,
      running: row.running,
      pending: pendingInteractions.has(row.id),
    ...
    for (const child of forkChildrenOf(row)) {
      childKeys.push(child.id)
      edges.push({ id: `fork:${row.id}->${child.id}`, from: row.id, to: child.id })
    }
    if (childKeys.length > 0) children.set(row.id, childKeys)
    for (const child of forkChildrenOf(row)) placeMember(child, clusterRootId, members)
  }
  ...
  return {
    ...
    children,
    edges,
    workspaceLabel: scope?.label,
    sessionCount: nodes.size,
  }
}

/**
 * The branch neighborhood of one canvas node: itself, its branch ancestors
 * up the {@link GraphNode.branchFrom} chain, and its fork descendants down
 ...
 * @returns the neighborhood ids.
 */
export function neighborhood(nodes: Iterable<GraphNode>, key: string): ReadonlySet<string> {
